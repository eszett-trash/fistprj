...
public class ShoesTailorView {
	...
	private void stockManagement() {
		...
		while (true) {
			...
			case "3":
				...
				System.out.println("이름을 수정 하실경우에는 이름을 입력 해 주세요\n"
						+ "입력하지 않으려면 null을 입력 해 주세요.");
				String name_s = sc.next();
				System.out.println("수량을 수정 하실경우에는 수량을 입력 해 주세요.\n"
								+ "입력하지 않으려면 0을 입력 해 주세요.");
				int amount_s = intErrchk();
				System.out.println("가격을 수정 하실경우에는 가격을 입력 해 주세요.\n"
						+ "입력하지 않으려면 0을 입력 해 주세요.");
				int price_s = intErrchk();
				System.out.println("색상을 수정 하실경우에는 색상을 입력 해 주세요\n"
								+ "입력하지 않으려면 null을 입력 해 주세요.");
				String color_s = sc.next();
				System.out.println("종류를 수정 하실 경우에는 종류를 입력 해 주세요\n"
						+ "입력하지 않으려면 null을 입력 해 주세요.");
				int kind_s = intErrchk();
				sv.update_stockList(stock_vo_id, name_s, amount_s, price_s,color_s, kind_s);
				break;
			case "#":
				admin();
			default:
	...
	private void salesManagement() {
		System.out.println(" ■■■■■■■■■■■■■■■■■ salesManagement ■■■■■■■■■■■■■■■■■ ");
		while (true) {
			System.out.println("\t #. 뒤로가기");
			view_salesList();
			switch (sc.next()) {
	...
	private void memberManagement() {
		System.out.println(" ■■■■■■■■■■■■■■■■■ memberManagement ■■■■■■■■■■■■■■■■■ ");
		while (true) {
			view_userList();
	...
	private void join() { // 회원가입
		while (true) {
			...
			while (true) {
				...
				String regEx01 = "^[a-zA-Z]{1}[a-zA-Z0-9_]{6,16}$";
				if (Pattern.matches(regEx01, account)) {
					System.out.println("사용할 수 있는 아이디 입니다.");
					break;
				} else {
					System.out.println("아이디 : 영문자로 시작, 영문자 또는 숫자의 조합으로 (8~16자) 다시 입력하세요. ");
				}
			}
			...
			switch (sc.next()) {
			case "1":
				sv.addLast_userList(account, password, deliveryAddress,cellphone, 0, purchaseGoods, false, true);
				login();
			case "2":
				break;
			case "#":
